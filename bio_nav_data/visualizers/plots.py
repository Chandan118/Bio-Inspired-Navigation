...
def plot_trajectory(df: pd.DataFrame, 
    ...
    try:
        ...
        # Plot trajectories
        ax.plot(df['ground_truth_x'], df['ground_truth_y'], 
        ...
        ax.plot(df['traditional_slam_x'], df['traditional_slam_y'],
               color=PlotConfig.COLORS['traditional_slam'],
               linestyle=PlotConfig.LINE_STYLES['traditional_slam'],
               linewidth=2, label='Traditional SLAM (Drift)', alpha=0.8)
        
        ax.plot(df['our_framework_x'], df['our_framework_y'],
               color=PlotConfig.COLORS['our_framework'],
               linestyle=PlotConfig.LINE_STYLES['our_framework'],
               linewidth=2.5, label='Our Framework (Corrected)', alpha=0.9)
        
        # Customize plot
        ax.set_title('Figure 13: Simulated Localization Accuracy', 
                    fontsize=PlotConfig.FONT_SIZE['title'], fontweight='bold')
        ...
        if show_legend:
            ax.legend(fontsize=PlotConfig.FONT_SIZE['legend'], 
...
def plot_energy_consumption(df: pd.DataFrame,
    ...
    try:
        ...
        # Create box plot
        box_plot = sns.boxplot(data=df, x='Method', y='Energy_Consumption_Watts', 
                              ax=ax, showfliers=show_outliers)
        
        # Customize plot
        ax.set_title('Figure 15B: Simulated Energy Consumption', 
                    fontsize=PlotConfig.FONT_SIZE['title'], fontweight='bold')
        ax.set_xlabel('Navigation Method', fontsize=PlotConfig.FONT_SIZE['axis_label'])
        ax.set_ylabel('Energy Consumption (Watts)', fontsize=PlotConfig.FONT_SIZE['axis_label'])
...
def _add_energy_statistics(ax: plt.Axes, df: pd.DataFrame) -> None:
    ...
    try:
        ...
        stats = df.groupby('Method')['Energy_Consumption_Watts'].agg(['mean', 'std']).round(1)
...
def plot_performance_comparison(df: pd.DataFrame,
    ...
    try:
        ...
        ax1.bar(x - width/2, df['Proposed_System_Value'], width, 
               label='Proposed System', color=PlotConfig.COLORS['our_framework'])
        ax1.bar(x + width/2, df['Baseline_ORB_SLAM3_Value'], width,
               label='Baseline ORB-SLAM3', color=PlotConfig.COLORS['traditional_slam'])
...
def create_combined_figure(trajectory_df: pd.DataFrame,
    ...
    try:
        ...
        # Plot 1: Trajectory
        ax1.plot(trajectory_df['ground_truth_x'], trajectory_df['ground_truth_y'],
                color=PlotConfig.COLORS['ground_truth'],
                linestyle=PlotConfig.LINE_STYLES['ground_truth'],
                linewidth=2, label='Ground Truth')
        
        ax1.plot(trajectory_df['traditional_slam_x'], trajectory_df['traditional_slam_y'],
                color=PlotConfig.COLORS['traditional_slam'],
                linestyle=PlotConfig.LINE_STYLES['traditional_slam'],
                linewidth=2, label='Traditional SLAM')
        
        ax1.plot(trajectory_df['our_framework_x'], trajectory_df['our_framework_y'],
                color=PlotConfig.COLORS['our_framework'],
                linestyle=PlotConfig.LINE_STYLES['our_framework'],
                linewidth=2, label='Our Framework')
