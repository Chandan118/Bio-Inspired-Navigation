...
def create_performance_summary() -> pd.DataFrame:
    ...
    try:
        ...
        lower_is_better = ['Localization Accuracy', 'Energy Consumption', 'Visual Occlusion Recovery', 
                          'Memory Usage', 'Real-time Performance']
        
        result_df['Is_Improvement'] = result_df.apply(
            lambda row: (row['Proposed_System_Value'] < row['Baseline_ORB_SLAM3_Value']) 
            if row['Metric'] in lower_is_better 
            else (row['Proposed_System_Value'] > row['Baseline_ORB_SLAM3_Value']), 
            axis=1
        )
        
        logger.info(f"Successfully created performance summary with {len(result_df)} metrics")
        return result_df
...
def validate_performance_data(df: pd.DataFrame) -> Dict[str, bool]:
    ...
    required_columns = ['Metric', 'Unit', 'Proposed_System_Value', 'Baseline_ORB_SLAM3_Value']
    validation['has_required_columns'] = all(col in df.columns for col in required_columns)
    
    # Check for non-negative values
    validation['non_negative_values'] = (
        (df['Proposed_System_Value'] >= 0).all() and 
        (df['Baseline_ORB_SLAM3_Value'] >= 0).all()
    )
    
    # Check for reasonable improvement percentages (-100 to 1000%)
    validation['reasonable_improvements'] = (
        (df['Improvement_Percentage'] >= -100) & 
        (df['Improvement_Percentage'] <= 1000)
    ).all()
