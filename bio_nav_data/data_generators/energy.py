...
class EnergyDataGenerator:
    ...
    def __init__(self, n_trials: int = 100, random_seed: int = 42):
        ...
        if n_trials < 10:
            raise ValueError(
                "n_trials must be at least 10 for statistical significance"
            )
        ...
        logger.info(
            f"EnergyDataGenerator initialized with {n_trials} trials per method"
        )
    ...
    def _generate_method_energy(self, method_name: str) -> np.ndarray:
        ...
        energy_data = np.random.normal(
            loc=params["loc"], scale=params["scale"], size=self.n_trials
        )
    ...
    def generate(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Successfully generated energy data for {len(self.energy_params)} methods"
            )
