...
class TrajectoryDataGenerator:
    ...
    def __init__(
        ...
        logger.info(
            f"TrajectoryDataGenerator initialized with {n_points} points, "
    ...
    def _generate_ground_truth(self) -> pd.DataFrame:
        ...
        return pd.DataFrame({"x": distances, "y": np.zeros(self.n_points)})
    ...
    def _generate_traditional_slam(self, ground_truth: pd.DataFrame) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {"x": ground_truth["x"] + x_drift, "y": ground_truth["y"] + y_drift}
        )
    ...
    def _generate_our_framework(self, ground_truth: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(1, self.n_points):
            if i % self.correction_interval == 0:
                ...
                our_framework_x[i] = ground_truth["x"].iloc[i] + np.random.normal(
                    0, 0.05
                )
                our_framework_y[i] = ground_truth["y"].iloc[i] + np.random.normal(
                    0, 0.2
                )
            else:
        ...
        return pd.DataFrame({"x": our_framework_x, "y": our_framework_y})
    ...
    def generate(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Successfully generated trajectory data with {len(result_df)} points"
            )
            return result_df
    ...
    def get_statistics(self, df: pd.DataFrame) -> dict:
        ...
        stats["improvement_ratio"] = (
            stats["traditional_slam_avg_drift"] / stats["our_framework_avg_error"]
        )
