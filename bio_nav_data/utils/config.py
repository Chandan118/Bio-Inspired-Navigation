...
class Config:
    ...
    def __init__(self, base_path: Optional[str] = None):
        ...
        self.trajectory_params: Dict[str, Number] = {
            "n_points": 101,
            "distance_m": 100.0,
        ...
        self.energy_params: Dict[str, int] = {
            "n_trials": 100,
            "random_seed": 42,
        }
    ...
    def validate_config(self) -> Dict[str, bool]:
        ...
        validation["trajectory_params_valid"] = (
            self.trajectory_params["n_points"] > 0
            and self.trajectory_params["distance_m"] > 0
            and self.trajectory_params["correction_interval"] > 0
        )

        validation["energy_params_valid"] = self.energy_params["n_trials"] > 0

        dpi = int(self.plot_params["dpi"])
        figsize = cast(Tuple[int, int], self.plot_params["figsize"])
        validation["plot_params_valid"] = (
            dpi > 0 and len(figsize) == 2 and all(size > 0 for size in figsize)
        )
    ...
    def print_summary(self) -> None:
        ...
        for key, param_value in self.trajectory_params.items():
            print(f"  {key}: {param_value}")

        print("\nEnergy Parameters:")
        for key, energy_value in self.energy_params.items():
            print(f"  {key}: {energy_value}")

        print("\nPlot Parameters:")
        for key, plot_value in self.plot_params.items():
            print(f"  {key}: {plot_value}")

        print("\nFile Names:")
        for key, file_value in self.file_names.items():
            print(f"  {key}: {file_value}")
