...
class BioNavDataRosNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.trajectory_publishers = {
            "ground_truth": self.create_publisher(
                PathMsg, "bio_nav_data/trajectory/path/ground_truth", 10
            ),
            "our_framework": self.create_publisher(
                PathMsg, "bio_nav_data/trajectory/path/our_framework", 10
            ),
            "traditional_slam": self.create_publisher(
                PathMsg, "bio_nav_data/trajectory/path/traditional_slam", 10
            ),
        }
        self.trajectory_json_pub = self.create_publisher(
            String, "bio_nav_data/trajectory/json", 10
        )
        self.energy_json_pub = self.create_publisher(
            String, "bio_nav_data/energy/json", 10
        )
        self.performance_json_pub = self.create_publisher(
            String, "bio_nav_data/performance/json", 10
        )
        self.analytics_json_pub = self.create_publisher(
            String, "bio_nav_data/analytics/json", 10
        )
    ...
    def _build_config(self) -> Config:
        ...
        trajectory_updates = {
            ...
            "correction_interval": int(
                self.get_parameter("trajectory.correction_interval").value
            ),
    ...
    def _build_analytics_payload(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        trajectory_stats = self.generator.trajectory_generator.get_statistics(
            data["trajectory"]
        )
        energy_stats = self.generator.energy_generator.get_statistics(data["energy"])
        energy_improvements = self.generator.energy_generator.calculate_improvements(
            data["energy"]
        )
        energy_quality = self.generator.energy_generator.validate_data_quality(
            data["energy"]
        )
    ...
    def _handle_generate_request(
        ...
        response.message = (
            ...
            if success
            else "Pipeline execution failed (see logs)."
        )
        return response
    ...
    def _publish_latest_data(self) -> None:
        ...
        self.trajectory_json_pub.publish(
            dataframe_to_json(self.latest_data["trajectory"])
        )
        self.energy_json_pub.publish(dataframe_to_json(self.latest_data["energy"]))
        self.performance_json_pub.publish(
            dataframe_to_json(self.latest_data["performance"])
        )
